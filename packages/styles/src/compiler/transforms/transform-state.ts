...
import { extractAtRuleParams } from '../syntax'
import type { StateTokenMetadata } from '../sheet'
...
import { hoistCondition, isHostMountedSelector } from '../hoist'
    ...
    if (ctx.isCombo) {
        ...
        for (const combo of comboList) {
            const comboMod = combo
                ...
                .join('')
            const replaced = replaceTargetInSelector(targetSelector, target, comboMod, true)
        ...
        for (const s of stateList) {
            const replaced = replaceTargetInSelector(targetSelector, target, s.modifier, false)
            const sel = replaced.result
