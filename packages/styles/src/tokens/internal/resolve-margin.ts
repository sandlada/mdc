...
import type { MarginObject } from '../expand-margin'

export const MARGIN_OBJECT_KEYS = new Set([
    'all',
    'block',
    'inline',
    'blockStart',
...
export function isMarginObject(val: unknown): val is MarginObject<any> {
    if (typeof val !== 'object' || val === null || Array.isArray(val)) {
        return false
    }
    const keys = Object.keys(val)
    return keys.some(k => MARGIN_OBJECT_KEYS.has(k))
}
...
export function resolveSingleMargin(val: unknown): ResolvedEdgeQuad {
    ...
    if (Array.isArray(val)) {
        ...
        throw new Error(
            `[expandMargin] Array input must have length 2 (axis [block, inline]) or 4 (edges [block-start, block-end, inline-start, inline-end]), got length ${val.length}.`
        )
    }
    ...
    throw new Error('[expandMargin] Invalid margin value.')
}
