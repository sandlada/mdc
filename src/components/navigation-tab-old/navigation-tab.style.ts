...
export const navigationTabStyle = [
    css`
        :host:has(.container.bar:not(.xr)) { ${barTokenString}; }
        :host:has(.container.rail:not(.xr)) { ${railTokenString}; }
        :host:has(.container.xr.bar) { ${barTokenString}; }
        :host:has(.container.xr.rail) { ${railTokenString}; }
    `,
    labelIconShared,
    indicatorShared,
    containerShared,
